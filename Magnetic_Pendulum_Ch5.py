...
mu = 0.1  # Magnetic moment (arbitrary units)
B0 = 0.1  # Magnetic field strength (T)
omega = 2.0  # Angular frequency of the magnetic field (rad/s)

# Initial conditions
r0 = 1.2  # Initial spring length (m)
theta0 = np.pi / 6  # Initial angle (rad)
...
plt.grid()
plt.legend()
plt.savefig('radial_phase_portrait.png')
...
plt.grid()
plt.legend()
plt.savefig('angular_phase_portrait.png')
...
ax.set_xlabel("X (m)")
ax.set_ylabel("Y (m)")
ax.set_title("Spring Pendulum with Magnetic Field")
line, = ax.plot([], [], 'o-', lw=2)
...
ani.save('spring_pendulum.gif', writer=PillowWriter(fps=30))
